...
describe("CRUD Operations", () => {
  ...
  describe("Patient CRUD Operations", () => {
    ...
    it("should get patient by email with valid inputs", async () => {
      const patientData = {
        firstName: "John",
        lastName: "Doe",
        dateOfBirth: new Date("1990-01-01").getTime(),
      ...
      const patient = await t.query(api.patients.getPatientByEmail, {
        email: "test.patient@example.com",
        tenantId: "tenant-1",
      });
    ...
    it("should return null for empty email", async () => {
      const result = await t.query(api.patients.getPatientByEmail, {
        email: "",
        tenantId: "tenant-1",
      });
    ...
    it("should return null for whitespace-only tenantId", async () => {
      const result = await t.query(api.patients.getPatientByEmail, {
        email: "test@example.com",
        tenantId: "   ",
      });
    ...
    it("should return null for email missing @ symbol", async () => {
      const result = await t.query(api.patients.getPatientByEmail, {
    ...
    it("should return null for email missing domain", async () => {
      const result = await t.query(api.patients.getPatientByEmail, {
        email: "test@",
        tenantId: "tenant-1",
      });
    ...
    it("should trim email and tenantId before querying", async () => {
      const patientData = {
        firstName: "John",
        lastName: "Doe",
        dateOfBirth: new Date("1990-01-01").getTime(),
        email: "trimmed.test@example.com",
        tenantId: "tenant-1",
      };

      await t.mutation(api.patients.createPatient, patientData);

      // Query with whitespace that should be trimmed
      const patient = await t.query(api.patients.getPatientByEmail, {
        email: "  trimmed.test@example.com  ",
        tenantId: "  tenant-1  ",
      });
      ...
      expect(patient).toBeDefined();
      expect(patient?.email).toBe("trimmed.test@example.com");
    });
    ...
    it("should handle errors gracefully", async () => {
      ...
      // We'll test with a valid format but non-existent patient
      const result = await t.query(api.patients.getPatientByEmail, {
