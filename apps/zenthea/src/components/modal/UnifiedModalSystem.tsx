...
import { Modal, ModalContextType, ModalStack, ModalPosition, ModalDimensions, TaskType, ModalPriority, ModalStatus } from './types';
import { useModal as useModalContext } from './ModalContext';
...
export function UnifiedModalProvider({ children }: { children: React.ReactNode }) {
...
function ModalWindow({ modal }: ModalWindowProps) {
  ...
  const [dimensions, setDimensions] = useState(modal.dimensions || { width: 500, height: 500 });
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {modal.status && (
            <Badge variant={modal.status === 'completed' ? 'default' : 'secondary'}>
              {modal.status}
            </Badge>
          )}
        </div>
...
// Event type configuration for medical modals
...
export function useMedicalModals() {
  const { openModal, closeModal, convertToTask } = useModal();
      ...
      content: (
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t">
            <Button 
              size="sm" 
              onClick={() => convertToTask(openModal({
...
// UnifiedModalSystem component that renders all active modals
