...
export default function WebsitePreviewPage() {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === 'PREVIEW_UPDATE') {
        ...
        logger.debug('[PreviewPage] Data validation:', {
        ...
        // Note: blocks can be empty array [], which is valid
        if (payload.templateId === undefined || payload.templateId === null ||
          ...
          logger.error('[PreviewPage] Invalid preview data structure:', {
            templateId: payload.templateId,
            header: payload.header,
            footer: payload.footer,
            theme: payload.theme,
            blocks: payload.blocks,
            blocksIsArray: Array.isArray(payload.blocks),
          });
          return;
        }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-secondary">
        <div className="text-center">
          {showError ? (
            <>
              <p className="text-text-secondary mb-2">Failed to load preview</p>
              <p className="text-sm text-text-tertiary">Please refresh the page or check the console for errors</p>
              <button
                onClick={() => {
                  window.location.reload();
                }}
                className="mt-4 px-4 py-2 bg-interactive-primary text-white rounded-md hover:bg-interactive-primary-hover"
              >
                Reload Page
              </button>
            </>
            ...
            <>
          <div className="animate-spin w-8 h-8 border-4 border-interactive-primary border-t-transparent rounded-full mx-auto mb-4" />
  ...
  logger.debug('[PreviewPage] Rendering with data:', {
    templateId: previewData.templateId,
    blocksCount: displayBlocks.length,
    hasHeader: !!previewData.header,
    hasFooter: !!previewData.footer,
    hasTheme: !!previewData.theme,
  });
  ...
  try {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-secondary">
        <div className="text-center">
          <p className="text-text-secondary mb-2">Error rendering preview</p>
