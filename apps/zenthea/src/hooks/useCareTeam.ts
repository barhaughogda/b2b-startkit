...
export function useCareTeam(patientId?: string) {
  ...
  const isUuid = patientId ? /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(patientId) : false;

  const { data, error, isLoading } = useSWR(
    session && patientId && isUuid ? `/api/patients/${patientId}/care-team` : null,
    fetcher
  );

  const careTeam: CareTeamMember[] = useMemo(() => (data?.members || []).map((m: any) => ({
    id: m.userId,
    userId: m.userId,
    name: m.name,
    role: m.role,
    ...
    source: m.source,
  })), [data?.members, data?.primaryProvider?.userId]);
