...
let GET: typeof import('@/app/api/admin/security-metrics/route').GET;

describe('GET /api/admin/security-metrics', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
    const routeModule = await import('@/app/api/admin/security-metrics/route');
    GET = routeModule.GET;
  });
  ...
  describe('Authentication & Authorization', () => {
    it('should return 401 if user is not authenticated', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/admin/security-metrics');
    ...
    it('should return security metrics for admin user', async () => {
      mockVerifyAdminAuth.mockResolvedValue({
        authorized: true,
  ...
  describe('Tenant Isolation', () => {
    it('should filter metrics by tenantId from session', async () => {
      ...
      mockConvexQueryFn.mockResolvedValue({ failedLoginAttempts: 2 });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle Convex query errors gracefully', async () => {
      ...
      expect(data.error).toBe('Failed to fetch security metrics');
    });
  });
});
